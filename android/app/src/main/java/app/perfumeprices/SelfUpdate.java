...
public class SelfUpdate extends BroadcastReceiver {
    ...
    private static final String APK = "perfume-prices.apk";
    ...
    static void start(Activity a) {
        if (!a.getPackageManager().canRequestPackageInstalls()) {
            ...
            Toast.makeText(a, "Дозволи да Cenovnik parfema инсталира ажурирања, па се врати", Toast.LENGTH_LONG).show();
            try {
        ...
        Toast.makeText(a, "Преузимам ажурирање…", Toast.LENGTH_SHORT).show();
        install(a, true);
    }
    ...
    static void install(Context ctx, boolean quiet) {
        ...
        new Thread(() -> {
            try {
                ...
                toast(app, "Преузимање није успело. Провери интернет.");
            }
        }).start();
    }
    ...
    public void onReceive(Context ctx, Intent intent) {
        ...
        if (status == PackageInstaller.STATUS_PENDING_USER_ACTION) {
            ...
            if (intent.getBooleanExtra(EXTRA_QUIET, false)) {
                ...
                Toast.makeText(ctx, "Отварам инсталацију…", Toast.LENGTH_SHORT).show();
                install(ctx, false);
            } else {
                Toast.makeText(ctx, "Ажурирање није успело. Покушај поново касније.", Toast.LENGTH_LONG).show();
            }
        }
    }
    ...
    private static void notifyUpdated(Context ctx) {
        ...
        nm.notify(77, new Notification.Builder(ctx, "updates")
                ...
                .setContentTitle("Cenovnik parfema је ажуриран")
                .setContentText("Додирни да отвориш нову верзију.")
